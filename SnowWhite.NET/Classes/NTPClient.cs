...
namespace TimeSync
{
    ...
    public class NTPClient
    {
        ...
        public NTPClient(string host, int port)
        {
            m_TimeServer = host;
            m_port = port;
        }
        ...
        public void Connect(bool UpdateSystemTime)
        {
            try {
                ...
                IPHostEntry hostadd = Dns.GetHostEntry(m_TimeServer);
                IPEndPoint EPhost = new IPEndPoint(hostadd.AddressList[0], m_port);
                ...
                if(!IsResponseValid())
                {
                    throw new Exception("Invalid response from " + m_TimeServer);
                }
